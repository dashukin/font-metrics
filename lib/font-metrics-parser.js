			...
			.evaluate(function (data) {
				...
				return new window.Promise(function (resolve, reject) {
					...
					window.Promise.all(fontsLoaded).then(function () {
						...
						fonts.forEach(function (fontData) {
							ctx.font = fontSize + ' ' + fontData.fontFamily;
...
FontMetricsParser.prototype.saveResult = function (data) {
	...
	return new Promise(function (resolve, reject) {
		...
		fs.writeFile(finalPath + 'fontsMap.json', JSON.stringify(data, null, 4), function (err) {
			if (err) {
				reject(err);
			} else {
				resolve();
...
FontMetricsParser.prototype.finish = function () {

	this.server.close();

};

FontMetricsParser.prototype.log = function (msg, type) {
